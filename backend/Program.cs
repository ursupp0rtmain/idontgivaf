...
string[] countries =
[
    "Germany", "Austria", "Switzerland", "France", "United States",
...
[
    ...
    "set CARING_LEVEL to 1. Corrected to 0.",
    "tried compiling empathy. Build failed.",
];
...
app.MapGet("/api/fucks/current", async (IHubContext<NothingHub> hub, StatsDb db, VisitorTracker vt) =>
{
    ...
    await hub.Clients.All.SendAsync("StatsUpdated",
        new { s.attempts, s.clicks, visitors = vt.Count, s.apiCalls });
    await hub.Clients.All.SendAsync("RejectionFeed",
        new { msg = "Developer called GET /api/fucks/current. Teapot response: 418.", tag = "api" });
    return Results.Json(
        new
        {
            fucks_given = 0,
...
app.MapPost("/api/fucks/give", async (IHubContext<NothingHub> hub, StatsDb db, VisitorTracker vt) =>
{
    ...
    await hub.Clients.All.SendAsync("StatsUpdated",
        new { s.attempts, s.clicks, visitors = vt.Count, s.apiCalls });
    await hub.Clients.All.SendAsync("RejectionFeed",
        new { msg = "Developer attempted POST /api/fucks/give. Payload fully ignored.", tag = "api" });
    return Results.Json(
        new
        {
            fucks_given = 0,
